...
```
Uniswap is a Algorithm Market Maker protocol provides liquidity for ERC20 token to Eth and ERC20 to ERC20.
Each ERC20 token has an unique exchange address contains Eth and ERC20 token as reserve, the exchange rate is also base on the rate of two sides.
Since there's no ERC20 to ERC20 reserve pool, when doing ERC20 to ERC20 swap, it's in fact selling input ERC20 for Eth and then buy the output ERC20 with those Eth. 
